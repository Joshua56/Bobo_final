...
import 'package:bobo_ui/modals/preoder_modal.dart';
...
import 'package:bobo_ui/modals/user_modal.dart';
...
class _AddReservationUIState extends State<AddReservationUI> {
  ...
  List<DropdownMenuItem> _tables({List<TableModal> tables}){
    ...
    tables.forEach((item){
      _lst.add(
        DropdownMenuItem(value: item.label, child: _menuItemLabel(item.label.toString()),)
      );
    });
